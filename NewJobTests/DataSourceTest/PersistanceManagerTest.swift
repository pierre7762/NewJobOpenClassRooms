...
final class PersistanceManagerTest: XCTestCase {
    ...
    //           return container
    //       }()
    //    var viewContextTest: NSManagedObjectContext {
    //        return self.persistentContainer.viewContext
    //    }
    //    var persistanceManagerTest = PersistenceManager(coreDataStack: FakeCoreDataStack())
    ...
    func testCreateInterview_WhenDataBaseHaveZeroInterview_ThenShouldReturnOne() {
        ...
        pmTest.createInterview(candidacyID: (selectedJob?.candidacy?.id)!, date: Date.now, comment: "Relaunch comment")
    ...
    func testFetchAllInterviewFromCandidacyId_WhenDataBaseHaveOneInterview_ThenShouldReturnZero() {
        ...
        let selectedJob = pmTest.fetchSelectedJobs().first
        pmTest.createInterview(candidacyID: (selectedJob?.candidacy?.id)!, date: Date.now, comment: "Relaunch comment")
        pmTest.createInterview(candidacyID: (selectedJob?.candidacy?.id)!, date: Date.now, comment: "Relaunch comment2")
