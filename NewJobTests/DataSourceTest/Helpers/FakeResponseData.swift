...
class FakeResponseData {
    ...
    static let url: URL = URL(string: "http://test.com")!

    static let responseOK = HTTPURLResponse(url: url, statusCode: 200, httpVersion: nil, headerFields: nil)!
    static let responseOK206 = HTTPURLResponse(url: url, statusCode: 206, httpVersion: nil, headerFields: nil)!
    ...
    static var correctDataLongApiGouvService: Data {
        let bundle = Bundle(for: FakeResponseData.self)
        let url = bundle.url(forResource: "apiGouvServiceResponseLong", withExtension: "json")
        let data = try! Data(contentsOf: url!)
        return data
    }
